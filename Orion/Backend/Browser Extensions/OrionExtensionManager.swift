...
class OrionExtensionManager {
  ...
  func loadAllExtensionData() {
    do {
      let extensions = try FileManager.default.contentsOfDirectory(atPath: FileManager.default.extensionDirectory!.path).filter {
        return FileManager.default.directoryExists(atPath: FileManager.default.extensionDirectory!.appendingPathComponent($0).path)
      }
      
      for ext in extensions {
        let manifest: MozExtManifest? = OrionConfig.readJSON(
          withUrl: FileManager.default.extensionDirectory!.appendingPathComponent(ext).appendingPathComponent("manifest.json"),
  ...
  func downloadExtension(withUrl url: URL) {
    ...
    do {
      ...
      if !FileManager.default.directoryExists(atPath: FileManager.default.extensionDirectory!.path) {
        try FileManager.default.createDirectory(at: FileManager.default.extensionDirectory!, withIntermediateDirectories: true)
      }
  ...
  private func installExtension(withName name: String) {
    ...
    do {
      try FileManager.default.createDirectory(at: extensionUnpackDest, withIntermediateDirectories: true, attributes: nil)
      ...
      let manifest: MozExtManifest? = OrionConfig.readJSON(withUrl: extensionUnpackDest.appendingPathComponent("manifest.json"), convertFromSnakeCase: true)
      guard manifest != nil else { throw MozManifestError.missing }
      
      try FileManager.default.moveItem(at: extensionUnpackDest, to: FileManager.default.extensionDirectory!.appendingPathComponent(manifest!.name))
