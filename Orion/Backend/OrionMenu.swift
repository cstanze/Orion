...
class OrionMenu: NSMenu {
  ...
  override init(title: String) {
    ...
    mainMenu.submenu?.items = [
      ...
      NSMenuItem(title: "Hide Others", action: #selector(NSApplication.hideOtherApplications(_:)), keyEquivalent: "h", modifier: [.command, .option]),
    ...
    // FIXME: These next 2 lines are hacky, fix please
    ...
    fileMenu.submenu?.items = [
      NSMenuItem(title: "New Window", action: #selector((NSApplication.shared.delegate! as! AppDelegate).newWindow), keyEquivalent: "n"),
      NSMenuItem.separator(),
      NSMenuItem(title: "Close Window", action: #selector((NSApplication.shared.delegate! as! AppDelegate).newWindow), keyEquivalent: "W")
    ]
    ...
    viewMenu.submenu?.items = [
      NSMenuItem(title: "Stop", action: #selector((NSApplication.shared.delegate! as! AppDelegate).stopReloadingCurrent), keyEquivalent: "."),
      NSMenuItem(title: "Reload Page", action: #selector((NSApplication.shared.delegate! as! AppDelegate).reloadCurrent), keyEquivalent: "r")
    ]
